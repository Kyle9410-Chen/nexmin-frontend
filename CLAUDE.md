...
| `DELETE /api/groups/{group_key}/members/{member_key}` | JWT + `admin` | `lib/request/removeGroupMember.ts` → `useUpdateMemberGroups` → `components/roster/EditGroupsDialog.tsx`, and `useRemoveMailingListMember` → `components/mailingList/RemoveMemberDialog.tsx` |

The backend calls this domain `googlegroup` and serves it under `/api/groups`, while this app calls it "mailing lists" throughout (route, hook, `queryKey`, types). That split is deliberate — the club-facing name is the mailing list — so **only the URL follows the backend**; do not rename the frontend domain to match. All three admin-only member routes are wired: `PATCH` behind the Role dropdown on `/mailing-lists/:groupKey` and `DELETE` behind the `UserMinus` button next to it, the same one the roster uses (both hidden for a non-admin, who would only ever get a 403), plus `POST`/`DELETE` behind the roster's group-editing dialog. The members table's own removal takes someone off **that one list**, which is why it is a separate hook from the roster's `useRemoveRosterMember`, which takes them off every list the club has; both dialogs warn on your own row, since which list gates sign-in is the backend's business and cannot be told apart here. It invalidates `["roster"]` and `["profile"]` alongside the group's member list — the roster carries each person's group keys, and `/my-groups` is the same membership seen from the other side.
...
- **`POST /api/users` takes `{ email, groups: [{ key, role }] }`.** The login group is always written and does not have to be named, so this side never needs its address; everything in `groups` goes on top of it, each with its own Google role. Naming the login group anyway is how a role is set on it, and `MANAGER`/`OWNER` there map onto this service's `admin` — which is why `AddMemberDialog` warns before saving whenever **any** list is given Manager or Owner: which one is the login group is the backend's business and cannot be told apart here.
- **Being on a list already is not an error, and there is no 409.** Keys and roles are validated before the first write, so a typo costs nothing; after that the operation is not atomic and nothing is rolled back, but every write is idempotent, so the recovery is to send the same request again. A 400 is a bad or repeated key and is worth showing verbatim.
- **`DELETE /api/users/{email}` removes every list, not just the login group** — that one alone would end their access while leaving them on the lists that carry the club's mail — and it takes the login group off last, so a half-finished removal leaves them visible on the roster. It is idempotent: an address on none of the lists still answers **204**, which is why `useRemoveRosterMember` no longer translates Google's `Missing required field: memberKey` and `RemoveMemberDialog` says "every club mailing list".
...
**`AddMemberDialog` puts someone on their lists in the same request.** Everything ticked in the picker goes into the `groups` of one `POST /api/users`; there is no second pass over the group routes, and the login group needs no special case, since ticking it is simply how its role is set. The optimistic row therefore reports `role: "member"` and only the keys that were named — the login group's key is unknown here, and whether the write grants admin depends on which list it is — with the refetch settling both.

**`EditGroupsDialog` edits roles as well as membership.** The roster carries keys but no roles, so the dialog reads each list the person is already on through `useMailingListMembersFor` — the same `["mailingList", key, "members"]` entries the members page uses, and on the backend the same member cache `RosterGroupKeys` builds the roster from, so a warm server makes no Google call. One Save carries three diffs: `add` (with the role to join as), `remove`, and `update` (role changes). The list-and-role control itself is `components/mailingList/GroupRolePicker.tsx`, shared with `AddMemberDialog`; it holds no membership state of its own, because what an unpicked role _means_ differs between the two — a real current role there, a plain MEMBER default here. **A role whose current value could not be read is never sent as a change** — the control falls back to Member, but that is a display default, and treating it as an intent would silently demote people. Role changes are applied optimistically into the group's member list too, since the dialog has just read that row.
